...
def worker(id, td3_trainer, rewards_queue, replay_buffer, max_episodes, max_steps, batch_size, explore_steps, \
    ...
    env = UnityEnv(env_name, worker_id=id+10, use_visual=False, use_both=True)
...
if __name__ == '__main__':
    ...
    model_path = './model/td3_threshold_random02'
    ...
    if args.train:
        ...
        while True:  # keep geting the episode reward from the queue
            ...
            if len(rewards)%50==0 and len(rewards)>0:
                plot(rewards)
                np.savetxt('rewards_thre_random02.txt', np.array(rewards)[:, np.newaxis], fmt='%.4f', newline=', ')
