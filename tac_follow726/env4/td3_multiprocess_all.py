...
def state_process(s, s0):
    ...
    return np.transpose([relative_x,relative_z]).reshape(-1)
...
if __name__ == '__main__':
    ...
    # model_path = './model/td3_all_random02'
    ...
    if args.train:
        ...
        while True:  # keep geting the episode reward from the queue
            ...
            if len(rewards)%50==0 and len(rewards)>0:
                plot(rewards)
                np.savetxt('rewards_all_random02.txt', np.array(rewards)[:, np.newaxis], fmt='%.4f', newline=', ')
