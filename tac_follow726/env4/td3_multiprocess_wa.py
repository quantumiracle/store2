...
def plot(rewards):
    ...
    plt.plot(rewards)
    plt.savefig('td3_multi_weighted_average.png')
    # plt.show()
...
if __name__ == '__main__':
    ...
    reward_scale = 1.0
    action_range=4.  # should not moving too fast to ensure relatively fast deformation
    state_dim = 3
    action_dim = 2
    hidden_dim = 128
    policy_target_update_interval = 3 # delayed update for the policy network and target networks
    DETERMINISTIC=True  # DDPG: deterministic policy gradient
    # model_path = './model/td3_weighted_average_random02'
    ...
    if args.train:
        ...
        while True:  # keep geting the episode reward from the queue
            ...
            if len(rewards)%50==0 and len(rewards)>0:
                plot(rewards)
                np.savetxt('rewards_wa_random02.txt', np.array(rewards)[:, np.newaxis], fmt='%.4f', newline=', ')
