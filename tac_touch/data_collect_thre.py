...
if __name__ == '__main__':
    ...
    env_name = "./tac_touch_random2"  # random range 0.2
    ...
    for eps in range(training_episodes):
        ...
        for step in range(episode_length):
            # plot(s)
            if step >0 and np.mean(np.abs(np.array(s[7:])-s0[7:]))>0.03 and s[4]+s[5]+s[6]!=0:  # set a threshold to extract deformation frames

                batch_s.append(state_process(s, s0))
                label_single= np.concatenate(([s[4]], [s[6]]))
                label_single=np.concatenate((s[1:4]/30., label_single))
                label=np.concatenate(([int(s[0])], label_single))
